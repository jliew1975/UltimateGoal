...
public class Dogeforia extends VuforiaLocalizerImpl {
    ...
    public void processFrame(Frame frame){
        if(frame != null ) {
            ...
            inputMat = new Mat(bitmap.getWidth(), bitmap.getHeight(), CvType.CV_8UC4);
            Utils.bitmapToMat(bitmap,inputMat);

            outMat = detector.processFrame(inputMat, null);

            if(showDebug) {
                if(loadedTrackableSets.size() > 0) {
    ...
    public void render() {
       ...
       // super.onRenderFrame()
        ...
        if(detector != null && dogeCVEnabled) {
            VuforiaLocalizerImpl.CloseableFrame frame = null;

            if(!getFrameQueue().isEmpty()) {
                try {
                    frame = getFrameQueue().take();
                    processFrame(frame);

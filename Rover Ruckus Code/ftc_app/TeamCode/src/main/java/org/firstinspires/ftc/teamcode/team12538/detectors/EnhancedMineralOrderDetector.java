...
public class EnhancedMineralOrderDetector extends DogeCVDetector implements MineralDetector {
    ...
    public double yMaxOffset = 0d;
    public double yMinOffset = 0d;

    public boolean disableSampling = false;

    public EnhancedMineralOrderDetector() {
        super();
        this.detectorName = "Mineral Order Detector";
    }
    ...
    public Mat process(Mat input) {
        ...
        double yActiveRegion = (getAdjustedSize().height / 2);
        double yMin = yActiveRegion - ((yActiveRegion / 2) + yMinOffset);
        double yMax = yActiveRegion + ((yActiveRegion / 2) + yMaxOffset);
        ...
        if(chosenYellowRect != null) {
            if(chosenYellowRect.y > yMin && chosenYellowRect.y < yMax) {
        ...
        Imgproc.line(workingMat,new Point(0, yMin), new Point(getAdjustedSize().width, yMin),new Scalar(0,255,0), 2);
        Imgproc.line(workingMat,new Point(0, yMax), new Point(getAdjustedSize().width,yMax),new Scalar(0,255,0), 2);
